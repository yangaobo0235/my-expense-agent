...
public class PromptGovernanceService {
    ...
    public PromptTemplate createDraft(
        ...
        if (repository.findByKeyAndVersion(required(promptKey, "promptKey"), required(version, "version")).isPresent()) {
            throw new MyExpenseAgentException(
                    MyExpenseAgentErrorCode.VALIDATION_FAILED, "Prompt key/version 已存在");
        }
    ...
    public PromptChangeRequest approve(UUID requestId, String comment, String actor) {
        ...
        if (request.status() != PromptChangeRequestStatus.PENDING) {
            throw new MyExpenseAgentException(MyExpenseAgentErrorCode.INVALID_STATE_TRANSITION, "审批单不是待审批状态");
        }
        if (sameActor(request.submittedBy(), actor)) {
            throw new MyExpenseAgentException(MyExpenseAgentErrorCode.VALIDATION_FAILED, "Prompt 提交人不能审批自己的变更");
        }
    ...
    public PromptChangeRequest reject(UUID requestId, String comment, String actor) {
        ...
        if (sameActor(request.submittedBy(), actor)) {
            throw new MyExpenseAgentException(MyExpenseAgentErrorCode.VALIDATION_FAILED, "Prompt 提交人不能驳回自己的变更");
        }
                ...
                .orElseThrow(
                        () ->
                                new MyExpenseAgentException(
                                        MyExpenseAgentErrorCode.VALIDATION_FAILED, "Prompt 不存在"));
    }
    ...
    private PromptChangeRequest changeRequest(UUID id) {
        return repository.findChangeRequest(id)
                .orElseThrow(
                        () ->
                                new MyExpenseAgentException(
                                        MyExpenseAgentErrorCode.VALIDATION_FAILED, "Prompt 审批单不存在"));
    }
    ...
    private static String required(String value, String field) {
        if (value == null || value.isBlank()) {
            throw new MyExpenseAgentException(MyExpenseAgentErrorCode.VALIDATION_FAILED, field + "不能为空");
        }
        return value.trim();
    }
    ...
    private static void requireComment(String comment, String action) {
        if (comment == null || comment.isBlank()) {
            throw new MyExpenseAgentException(
                    MyExpenseAgentErrorCode.VALIDATION_FAILED, action + " Prompt 时必须填写意见");
        }
    }
