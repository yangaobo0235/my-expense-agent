...
class LlmExpenseDocumentExtractorTest {
    ...
    @Test
    void shouldUseDeterministicFallbackWhenApiKeyIsMissing() {
        ...
        assertThat(candidate.extractorMode()).isEqualTo("llm-fallback");
        assertThat(candidate.document().sellerName()).isEqualTo("Hotel");
    }

    @Test
    void shouldFailFastWhenApiKeyAndFallbackAreDisabled() {
        ExpenseExtractionProperties properties = new ExpenseExtractionProperties();
        properties.setApiKey("");
    ...
    void shouldPreferConfiguredExtractionModelOverPromptModel() {
        ...
        properties.setModelName("gpt-5.4");
        LlmExpenseDocumentExtractor extractor =
                new LlmExpenseDocumentExtractor(
                        properties,
                        new ExpenseExtractionJsonSchema(),
        ...
        String selectedModel =
                extractor.extractionModelName(
                        new RenderedPrompt(
                                ...
                                "prompt",
                                "hash",
                                "prompt-model-must-not-override-extraction-model",
                                BigDecimal.ZERO,
                                1024));

        assertThat(selectedModel).isEqualTo("gpt-5.4");
    }
