...
describe('consumeCaseEvents', () => {
  ...
  it('clears stale cursor and reconnects after server requests reset', async () => {
    sessionStorage.setItem('my-expense-agent-event:case-1', 'stale-event');
    ...
    expect(String(vi.mocked(fetch).mock.calls.at(-1)?.[0])).toContain(
      '/api/v1/fund-applications/case-1/events',
    );
    expect(sessionStorage.getItem('my-expense-agent-event:case-1')).toBe('event-2');
  });
});
